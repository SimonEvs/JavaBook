...
public class N17ReferenceTypesAndCopying {
    public static void main(String[] args) {

        // Ссылочные типы и копирование объектов
        // При работе с объектами классов надо учитывать, что они все представляют
        // ссылочные типы, то есть указывают на какой-то объект, расположенный в памяти.
        // Чтобы понять возможные трудности, с которыми мы можем столкнуться, рассмотрим
        // пример:
...
// Здесь создаем два объекта Person и один присваиваем другому. Но, несмотря на
// то, что мы изменяем только объект bob, вместе с ним изменяется и объект tom.
...
// переменной bob, например, с помощью метода clone:
...
// Для реализации клонирования класс Person должен применить интерфейс
// Cloneable, который определяет метод clone. Реализация этого метода просто
// возвращает вызов метода clone для родительского класса - то есть класса
// Object с преобразованием к типу Person.
...
// клонируемый объект не содержит сложных объектов. Например, пусть класс Book
...
// }
// В этом случае, хотя переменные book и book2 будут указывать на разные объекты
// в памяти, но эти объекты при этом будут указывать на один объект Author.

// И в этом случае нам необходимо выполнить полное копирование. Для этого надо

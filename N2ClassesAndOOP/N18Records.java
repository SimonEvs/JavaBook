...
record Person(String name, int age) { 
    ...
    // В данном случае если передано невалидное значение, то применяем некоторое
    // значение по умолчанию (число 18). В итоге фактически мы получим конструктор
...
Person(String name, int age) { 
    ...
    // Также мы можем определять какие-то другие конструкторы, но все они должны
    // вызывать канонический конструктор:
    ...
    // пользователя. Этот конструктор вызывает канонический конструктор, передая ему
    ...
    // Также мы можем переопределить методы, которые имеет record по умолчанию. А
    // это методы equals(), hashCode() и toString() и методы, которые называются
    // также, как и поля записи, и которые возвращают значения соответствующих
    // полей.
    ...
    // Ограничения records
    // Следует учитывать, что мы не можем наследовать запись record от других
    // классов. Также нельзя наследовать классы от records. Однако классы record
    // могут реализовать интерфейсы. Кроме того, классы record не могут быть
    // абстрактными.

    // В record нельзя явным образом определять нестатические поля и инициализаторы.
    // Но можно определять статические переменные и инициализаторы, также как
